...
__all__ = (
    ("composite_op_lookup", "validate_operator") + image_operators + array_operators
)
...
def validate_operator(how, is_image):
    ...
    if is_image:
        if name not in image_operators:
            raise ValueError(
                "Operator %r not one of the supported image operators: %s"
                % (how, ", ".join(repr(el) for el in image_operators))
            )
    elif name not in array_operators:
        raise ValueError(
            "Operator %r not one of the supported array operators: %s"
...
@operator
def multiply(src, dst):
    sr, sg, sb, sa = extract_scaled(src)
    dr, dg, db, da = extract_scaled(dst)

    a = sa + da * (1 - sa)

    f = lambda sx, dx: sx * dx
    r = (1 / a) * (
        (1 - da) * (sr * sa) + (1 - sa) * (dr * da) + (sr * sa) * (dr * da) * f(sr, dr)
    )
    g = (1 / a) * (
        (1 - da) * (sg * sa) + (1 - sa) * (dg * da) + (sg * sa) * (dg * da) * f(sg, dg)
    )
    b = (1 / a) * (
        (1 - da) * (sb * sa) + (1 - sa) * (db * da) + (sb * sa) * (db * da) * f(sb, db)
    )
    return combine_scaled(r, g, b, a)
